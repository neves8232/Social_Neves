...
def upload(request):
    if request.method == 'POST':
        ...
        new_post = Post.objects.create(user=user, image=image, caption=caption, img_url=img_url)
        new_post.save()
...
def profile(request, pk):
    ...
    user_following = len(FollowersCount.objects.filter(user=follower))
    ...
    if request.method == 'POST':
        ...
        if password == password2:
            if User.objects.filter(email=email).exists():
                messages.info(request, 'Email já está a ser usado morcão')
                return redirect('signup')
            elif User.objects.filter(username=username).exists():
                messages.info(request, 'Já foste, o user já foi escolhido por outro')
                return redirect('signup')
            else:
                user = User.objects.create_user(username=username, email=email, password=password)
                user.save()

                # logar o user e redirecionar para a pagina settings
                user_login = auth.authenticate(username=username, password=password)
                auth.login(request, user_login)

                #criar profile object para o novo utilizador
                user_model = User.objects.get(username=username)
                new_profile = Profile.objects.create(user=user_model, id_user=user_model.id)
                new_profile.save()
                return redirect('settings')
        else:
            messages.info(request, 'As passwords não são iguais Zé')
            return redirect('signup')
    else:
